...
function startLoading(){
	var btn = $('from').find('button[type=submit]');
	var icon = btn.find('i');
...
function stopLoading(){
	var btn = $('form').find('button[type=summit]');
	var icon = btn.find('i');
...
function gotoPage(index) {
	if (index) {
		...
		var hasPageParm = search.search(/page\=\d+\&?/)!==(-1);
		if (hasPageParm) {
...
function showConfirm(title, text, fn_ok, fn_cancel) {
	var s = '<div id="div-confirm" class="uk-modal>' + 
			...
			'&nbsp;&nbsp;&nbsp;' + 
			'<button class="uk-button uk-button-primary x-ok"><i class="uk-icon-times"></i> 否</button>' + 
			'</p></div></div>';
	$('body').append(s);
	var m = $('#div-confirm');
	var modal = new $.UIkit.modal.Model('#div-confirm');
