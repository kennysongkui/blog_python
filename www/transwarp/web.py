...
# thread local object for storing request and respones;
...
class Dict(dict):
	...
	Traceback (most recent call last):
		...
	AttributerError: 'Dict' object has no attribute 'empty'
	>>> d3 = Dict(('a', 'b', 'c'), (1, 2, 3))
	>>> d3.a
	1
	>>> d3.b
	...
	def __getattr__(self, key):
		try:
			...
			raise AttributerError(r"'Dict' object has no attribute '%s'" % key)
...
_RE_RESPONSE_STATUSES = re.compile(r'^\d\d\d(\ [\w\ ]+)?$')
...
class HttpError(Exception):
	...
	def __init__(self, code):
		...
		self.statues = '%d %s' % (code, _RESPONSE_STATUSES[code])
...
class RedirectError(HttpError):
	...
	def __str__(self):
		return '%s, %s' % (self.statues, self.location)
...
def redirect(location):
	...
	>>> raise redirect('http://www.test.com/')
	Traceback (most recent call last):
		...
	RedirectError: 301 Moved Permanently, http://www.test.com/
	'''
...
def found(location):
	...
	>>> raise found('http://www.test.com/')
	Traceback (most recent call last):
		...
	RedirectError: 302 Found, http://www.test.com/
	'''
...
def seeother(location):
	...
	>>> raise seeother('http://www.test.com/')
	Traceback (most recent call last):
		...
	RedirectError: 303 See Other, http://www.test.com/
	>>> e = seeother('http://www.test.com/seeother?r=123')
	>>> e.location
	'http://www.test.com/seeother?r=123'
	'''
...
class Request(object):
	...
	def _parse_input(self):
		...
		fs = cgi.FieldStorage(fp=slef._environ['wsgi.input'], environ=self._environ, keep_blank_values=True)
		inputs = dict()
		for key in fs:
	...
	def __getitem__(self, key):
		...
		>>> from StringIO import StringIO
		>>> r = Request({'REQUEST_METHOD':'POST', 'wsgi.input':StringIO('a=1&b=M%20M&c=ABC&c=XY&e=')})
		>>> r['a']
		u'1'
		>>> r['c']
		u'ABC'
		...
		>>> payload = '\\n'.join(p1)
		>>> r = Request({'REQUEST_METHOD':'POST', 'CONTENT-LENGTH':str(len(payload)), 'CONTENT_TYPE':'multipart/form-data; boundary=%s' % b, 'wsgi.input':StringIO(payload)})
		>>> r.get('name')
		u'Scofield'
		>>> r.gets('name')
	...
	def input(self, **kw):
		'''
		Get input as dict from request, fill dict using provided default values if key not exist.
	...
	def get_body(self):
		...
		'<xml><raw/>
		'''
		fp =  slef._environ['wsgi.input']
		return fp.read()

	@property
	def remote_addr(self):
		'''
		Get remote addr. Return '0.0.0.0' if cnanot get remote_addr.
	...
	def query_string(self):
		'''
		Get raw query string as str. Return '' if no query sting.
	...
	def request_method(self):
		...
		'POST'
		'''
		returned self._environ['REQUEST_METHOD']
	...
	def headers(self):
		'''
		Get all HTTP headers with key as str and value as unicode. The  header name are 'XXX-XXX' uppercase.
		...
		if not hasattr(self, '_cookies'):
			...
			if cookie_str:
				for c in cookie_str.split(';'):
					pos = c.find('=')
					if pos>0:
						cookies[c[:pos].stip()] = _unquote(c[pos+1:])
			self._cookies = cookies
		return slef._cookies
...
class Response(object):
	...
	def set_cookie(self, name, value, max_age=None, path='/', domain=None, secure=False, http_only=True):
		'''
		Set a cookie.
		...
		Args:
			...
			expires: optional, unix timestamp, datetime or date object that indicate an absolute time of the
					expiration time of cooke. Note that if expires specified, the max_age will be ignored.
		...
		>>> r._cookies
		{'company': 'company=Abc%2C%20Inc.; Max-Aget=3600; path=/; HttpOnly'}
		>>> r.set_cookie('company', r'Exapmle="Limited"', expires=1342274794.123, path='/sub/')
		>>> r._cookies
		...
		>>> r._cookies
		{'company': 'company=Expires; Expires=Sat, 14-Jul-2014 14:06:34 GMT; Path=/; HttpOnly'}
		'''
		if not hasattr(self, '_cookies'):
			self._cookies = {}
	...
	def status(self, value):
		...
		Traceback (most recent call last):
			...
		ValueError: Bad Response cade: 99
		>>> r.status = 'ok'
		Traceback (most recent call last):
			...
		...
		if isinstance(value, (int, long)):
			...
			if _RE_RESPONSE_STATUSES.match(value):
				self._status = value
			else:
...
class Jinja2TemplateEngine(TemplateEngine):
	...
	>>> engine('jinja2-test.html', dict(name='Test', posted_at=datetime.datetime(2014, 6, 1, 10, 11, 12)))
	'<p>Hello, Test.</p><span>2014-06-01 10:11:12</span>'
	'''
...
def _default_error_handler(e, start_response, is_debug):
	if isinstance(e,  HttpError):
		...
		return ('<html><body><h1>%d</h1></body></html>' % e.status)
	logging.exception('Exception:')
	start_response('500 Internal Server Error', [('Content-Type', 'text/html'), _HEADER_X_POWERED_BY])
	if is_debug:
		return _debug()
	return ('<html><body><h1>500 Internal Error</h1><h3>%s</h3></body></html>' % str(e))
	...
	def get_wsgi_application(self, debug=False):
		...
		def wsgi(env, start_response):
			...
			try:
				...
				start_response('500 Internal Server Error', [])
				return [
					r'''<html><body><h1>500 Internal Server Error</h1><div style="font-family:Monaco, Menlo, Consolas, 'Couyrier New', monospace;"><pre>''',
